(() => {
  ...
  async function startCameraScan() {
    ...
    info.textContent = 'Kamera wird vorbereitet …';
    if (!('mediaDevices' in navigator) || !navigator.mediaDevices.getUserMedia) {
      info.textContent = 'Kamera nicht verfügbar. Bitte Code manuell eingeben.';
      return;
    }
    try {
      ...
      if (!('BarcodeDetector' in window)) {
        info.textContent = 'Dieser Browser unterstützt den QR-Scanner nicht. Bitte Code manuell eingeben.';
        return;
      }
      const detector = new BarcodeDetector({ formats: ['qr_code'] });
      info.textContent = 'Kamera aktiv. QR-Code vor die Kamera halten.';
      ...
      info.textContent = 'Kamera konnte nicht gestartet werden. Bitte Berechtigung prüfen oder Code manuell eingeben.';
    }
  }
  ...
  function initCodes() {
    ...
    senseList().forEach(sense => {
      ...
      card.innerHTML = `<h2>${sense.label}</h2><img src="assets/qrcodes/${sense.id}.png" alt="QR-Code ${sense.label}"><div class="code">${sense.code}</div>`;
