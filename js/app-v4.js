(() => {
  ...
  function initMiniGame() {
    ...
    function updateSprite(now) {
      ...
      if (velocity < 0) setSprite((Math.floor(now / 240) % 2 === 0) ? SPRITES.left1 : SPRITES.left2);
      else if (velocity > 0) setSprite((Math.floor(now / 240) % 2 === 0) ? SPRITES.right1 : SPRITES.right2);
      else setSprite(direction < 0 ? SPRITES.left1 : SPRITES.right1);
    ...
    function heroHitbox() {
      ...
      return {
        left: heroX - heroW * 0.20,
        right: heroX + heroW * 0.20,
        top: top + heroH * 0.24,
        bottom: bottom - 6
      };
    }
    function orbHitbox(orb) {
      return {
        left: orb.x + orb.size * 0.16,
        right: orb.x + orb.size * 0.84,
        top: orb.y + orb.size * 0.16,
        bottom: orb.y + orb.size * 0.84
      };
    }
    function hedgehogHitbox() {
      const d = Math.max(16, Math.min(24, Math.min(hedgehog.width, hedgehog.height) * 0.34));
      const cx = hedgehog.x + hedgehog.width * 0.52;
      const cy = stageH - 6 - hedgehog.height * 0.42;
      return {
        left: cx - d / 2,
        right: cx + d / 2,
        top: cy - d / 2,
    ...
    function setHedgehogFrame(now, force=false) {
      if (!force && now - hedgehog.lastFrameSwap < HEDGE_FRAME_MS) return;
      if (!force) hedgehog.frame = (hedgehog.frame + 1) % 2;
      const frames = hedgehogFrames(hedgehog.direction);
      const src = frames[hedgehog.frame];
      if (hedgehogNode.src !== src) hedgehogNode.src = src;
      const natW = hedgehogNode.naturalWidth || 240;
      const natH = hedgehogNode.naturalHeight || 170;
      hedgehog.height = hedgehog.width * (natH / Math.max(1, natW));
      hedgehog.lastFrameSwap = now;
    }
    function startHedgehogRun(now) {
      hedgehog.phase = 'running';
      hedgehog.width = Math.round(localClamp(stageW * 0.105, 70, 90));
      hedgehog.speed = localClamp(stageW * 0.20, 170, 215);
      hedgehog.frame = 0;
      hedgehog.lastFrameSwap = now;
      hedgehog.x = hedgehog.direction < 0 ? stageW + hedgehog.width : -hedgehog.width;
      hedgehogNode.style.display = 'block';
      hedgehogNode.style.width = `${hedgehog.width}px`;
      hedgehogNode.style.left = '0';
      hedgehogNode.style.bottom = '6px';
      setHedgehogFrame(now, true);
      hedgehogNode.style.transform = `translate3d(${Math.round(hedgehog.x)}px, 0, 0)`;
    }
    function updateHedgehog(dt, now) {
      ...
      hedgehogNode.style.transform = `translate3d(${Math.round(hedgehog.x)}px, 0, 0)`;
      setHedgehogFrame(now);
      const noseVisible = hedgehog.direction < 0 ? hedgehog.x < stageW : (hedgehog.x + hedgehog.width) > 0;
      if (noseVisible) {
        warningLeft.classList.remove('active');
        warningRight.classList.remove('active');
      }
      const offLeft = hedgehog.direction < 0 && hedgehog.x < -hedgehog.width - 20;
      const offRight = hedgehog.direction > 0 && hedgehog.x > stageW + hedgehog.width + 20;
      if (offLeft || offRight) {
        hedgehog.phase = 'idle';
        hedgehogNode.style.display = 'none';
        warningLeft.classList.remove('active');
      ...
      if (!gameOver && !gameWon && intersects(heroHitbox(), hedgehogHitbox())) damageHero('bottom');
    }
