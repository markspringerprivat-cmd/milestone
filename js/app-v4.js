(() => {
  ...
  function createJourneyPathSvg(state) {
    ...
    pairs.forEach(([fromNode, toNode]) => {
      ...
      line.setAttribute('class', 'journey-path');
      svg.appendChild(line);
    });
    if (pendingJourneyReveal?.from != null && pendingJourneyReveal?.to != null) {
      const from = boardPointForNode(pendingJourneyReveal.from);
      const to = boardPointForNode(pendingJourneyReveal.to);
      const line = document.createElementNS(svgNS, 'line');
      line.setAttribute('x1', String(from.x));
      line.setAttribute('y1', String(from.y));
      line.setAttribute('x2', String(to.x));
      line.setAttribute('y2', String(to.y));
      line.setAttribute('class', 'journey-path journey-path--active');
      svg.appendChild(line);
    }
    return svg;
  }
  ...
  function islandKeyBadge(id) {
    ...
    return `<span class="board-island-key-badge"><img src="${assetUrl(BIOME_BY_SENSE[id].key)}" alt="${esc((JOURNEY_LABELS[id] || id) + '-Schlüssel')}"></span>`;
  }
  ...
  function createIslandButton(node, type, state) {
    ...
    btn.style.left = `${pos.x}%`;
    btn.style.top = `${pos.y}%`;
    const floating = String(currentBoardNode(state)) === String(node);
    if (floating) btn.classList.add('is-current');
    if (pendingJourneyReveal && String(pendingJourneyReveal.to) === String(node)) btn.classList.add('is-newly-revealed');
    if (type === 'start') {
      ...
      btn.innerHTML = `
        ...
        <span class="board-island-pill start-pill">START</span>
        <span class="board-island-market ${canScan ? 'is-guided' : 'is-disabled'}"><img src="${assetUrl('assets/images/ui/market_board.png')}" alt="Marktbrett"></span>`;
    ...
    btn.innerHTML = `
      ...
      <span class="board-island-pill level-pill">${String(node)}</span>
      <span class="board-island-caption">${esc(JOURNEY_LABELS[type] || BIOME_BY_SENSE[type]?.label || type)}</span>
  ...
  function updateBoardStatusText(node, state) {
    ...
    node.innerHTML = '<strong>Ziel:</strong> Scanne am Marktbrett auf der Startinsel einen Steckbrief.';
  }
  ...
  function renderBoard() {
    ...
    hero.classList.toggle('is-floating-companion', true);
  ...
  async function unlockSense(id, index) {
    ...
    state.boardCurrentNode = toNode;
    setState(state);
    pendingJourneyReveal = { from: fromNode || 'start', to: toNode, senseId: id };
    renderBoard();
    playSound('levelunlocked');
    playSound('background', { loop:true, restart:false });
  }
